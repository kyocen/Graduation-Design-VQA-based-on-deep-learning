...
parser.add_argument("-num",type=int,action="store",help="gpu_index",default=2)
...
# feature save to disk 把图片转为feature 将每个batch的image feature存成dict_batchID.npy 保存一个dict: image_id->image feature
    ...
    for i,filepath in enumerate(glob.glob(pattern),1):
        ...
        if i%bs==0:
            ...
            for image_id, feature in zip(image_ids,output):
                dict[image_id]=feature
            np.save(os.path.join(f_dir,"dict_"+str(batch_id)),dict)# Save an array to a binary file in NumPy .npy format.
...
if __name__=="__main__":
    torch.cuda.set_device(args.num)
    #############################################################################
    # get offine images features
    get_image_feature("../data/vqa02/train2014","../data/vqa02/train2014_feature",BATCH_SIZE)
    get_image_feature("../data/vqa02/val2014","../data/vqa02/val2014_feature",BATCH_SIZE)
