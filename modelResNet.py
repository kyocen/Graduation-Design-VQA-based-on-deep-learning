...
def loss_acc(minibatch, net, criterion):
    ...
    for i, batch in enumerate(minibatch, 1):
        ...
        # print("[Y]: ", Y)
        # print("[y_hat]: ", y_hat)
        # ndy_hat=(y_hat.data).cpu().numpy()
        # dnY=(Y.data).cpu().numpy()
        # print(type(dnY),dnY)
        # print(type(ndy_hat),ndy_hat)
        # print(dnY.shape)
        # print(ndy_hat.shape)
        #
        # print("[reshape]",y_hat.data.view(-1, 4))
        # print("[reshape]",Y.data.view(-1, 4))
        # print(type(y_hat), y_hat)
        # print(type(Y), Y)

        values, ids = torch.max(y_hat.data.view(-1, 4), 1)  # 共有4个答案，最后一个为正确答案，将model给出的答案index vector转为0/1 vector
        values_c, ids_c = torch.max(Y.data.view(-1, 4), 1)
...
if __name__ == '__main__':
    ...
    # get_resnet101_feature('../dataset/Visual7W/ResizeImages/', '../dataset/Visual7W/imageFeature224/', BATCH_SIZE)
    ...
    # load image feature
    f_dict = load_feature('../dataset/Visual7W/imageFeature224/')
