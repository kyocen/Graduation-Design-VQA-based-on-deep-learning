...
class mfb_coatt_glove(nn.Module):
    ...
    def forward(self, data, word_length, img_feature, glove, mode):
        ...
        qatt_conv2 = qatt_conv2.view(self.batch_size*self.opt.NUM_QUESTION_GLIMPSE,-1)# 2N x T
        qatt_softmax = self.Softmax(qatt_conv2)#对T做softmax
        qatt_softmax = qatt_softmax.view(self.batch_size, self.opt.NUM_QUESTION_GLIMPSE, -1, 1)# N x 2 x T x 1
        qatt_feature_list = []
